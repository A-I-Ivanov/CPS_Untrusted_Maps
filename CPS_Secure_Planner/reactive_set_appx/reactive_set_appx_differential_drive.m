...
R = [0 0 0 0 0 0 0 0;   %Timing cost
     0 3 0 0 0 0 0 0;   %My state cost. Dont turn too much
...
xStart = [0,-.2, pi/2, 1.5, 0]'; %Where and how fast am I going?
...
ub(nx+nu+1:nx+nu+1:end) = 10; %%Bound the turn rate
lb(nx+nu+1:nx+nu+1:end) = -10;


%Some options for the optimizer
opts = optimset('Display','iter','Algorithm','sqp', 'MaxIter', 100000, 'MaxFunEvals', 100000,'TolX', 1e-17);
...
%plot(result(2:(nx+nu+1):end), result(3:(nx+nu+1):end))
axis([-1 1 -1 1])
